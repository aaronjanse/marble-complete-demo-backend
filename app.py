...
from marblecomplete.marbles.chars import *
from marblecomplete.marbles.constants import LEFT_TILT, RIGHT_TILT
...
async def handle_sockets(websocket, path):
    ...
    def on_microtick(dot):
        ...
        time.sleep(0.2)  # small delay for autostep

        debug_lines = 80

        d_l = []
        for idx in reversed(range(len(interpreter.env.marbles))):
            d = interpreter.env.marbles[idx]
            if d:
        ...
        for y in range(len(interpreter.env.world.map)):
            ...
            if len(''.join(c.literal for c in interpreter.env.world.map[y]).rstrip()) < 1:
                if last_blank:
                    continue
                else:
                    last_blank = True
            ...
            for x in range(len(interpreter.env.world.map[y])):
                ...
                if char == ' ':
                    ...
                    pending_txt += '\033[0;31m' + char.literal + '\033[0m'
                elif char.isToggler():
                    if char.tilt == LEFT_TILT:
                        display_char = 't' if char.is_ascii else '↘'
                    elif char.tilt == RIGHT_TILT:
                        display_char = 'T' if char.is_ascii else '↙'
                    pending_txt += '\033[0;34m' + display_char + '\033[0m'
                elif char.isGate():
                    pending_txt += '\033[0;33m' + char.literal + '\033[0m'
                elif char.isWire() and char.is_active:
                    pending_txt += '\033[0;32m' + char.literal + '\033[0m'
                    char.is_active = False
                elif char != '\n':
                    pending_txt += char.literal  # White
        ...
        while True:
            ...
            if instruction == 'run':
                ...
                try:
                    env = Env()
                    env.io = io_callbacks
                    interpreter = AsciiMarblesInterpreter(env, program, './marblecomplete')
                except Exception as e:
